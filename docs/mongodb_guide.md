...
The `mongodb_tls` roles does not generate SSL certificates.  They must be generated by the user and put in the top level `files` directory on the Ansible control node. These files are uploaded to the location defined in `mongodb_ssl_root_dir`.
